...
export class Heatmap<V extends string, X extends string, Y extends string> {
    ...
    /**
     * All configuration properties including default values.
     */
    _config: HeatmapConfiguration<V, X, Y>;

    _renderer: Canvas2dRenderer;

    /**
     * Use this functionality only for adding data points on the fly, not for data
     * initialization! heatmapInstance.addData adds a single or multiple
     * data points to the heatmap's datastore.
     *
     ...
     *
     * @example <caption>multiple data points</caption>
     *
...
export interface BaseHeatmapConfiguration<V extends string = 'value'> {
    ...
    /**
     * The blur factor that will be applied to all data points. The higher the
...
export interface HeatmapConfiguration<
    ...
    /**
     * Canvas element. If no canvas element is provided, it will be created.
     */
 ...
 */
export interface HeatmapData<T = DataPoint> {
    /**
...
declare class Canvas2dRenderer {
    ...
    _prepareData(data: HeatmapData)

    renderPartial(data: HeatmapData): void;

    renderAll(data: HeatmapData): void;
    ...
    _drawAlpha(data: HeatmapData): void;
